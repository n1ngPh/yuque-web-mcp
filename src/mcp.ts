...
能力发现规则：任何工作流开始前优先调用 yuque_get_capabilities。availability=disabled 的能力不得尝试；preview_only 只允许生成本地Diff，不代表可以远程Confirm。WRITE_CONSISTENCY_MODE默认strict，缺少可靠并发保护时远程Confirm会在发包前失败关闭；已有知识库内的写入只有部署者显式启用best_effort、设置WRITE_KILL_SWITCH=false，并按目标Host启用YUQUE_WRITE_ORGANIZATION_OPEN=true或YUQUE_WRITE_PERSONAL_OPEN=true时才允许进入已验证写契约；实际权限仍由当前语雀账号决定。Agent不得自行修改部署配置。
...
删除边界规则：Doc、Sheet和个人知识库整对象删除工具存在，但默认关闭；只有部署者显式开启ALLOW_OBJECT_DELETION=true、YUQUE_WRITE_PERSONAL_OPEN=true且专用个人Host契约完成真实捕获、关闭浏览器重放和删除后对账时才生成Preview。Preview必须展示完整路径和不可恢复影响；Confirm除diff_digest及confirm_deletions=true外还必须原样提交完整路径confirmation_text。非空知识库还必须在Preview显式allow_nonempty=true。DELETE方法本身不等于资源删除，例如DELETE /lock仅释放临时协作锁。
...
统计文档创建人和创建时间时，先定位候选文档，再调用yuque_get_doc_metadata读取元数据，按creator.id和created_at过滤；禁止扫描正文人名或以updated_at代替创建时间。缺失或失败项须单独报告。
...
export const toolDefinitions: ToolDefinition[] = [
  ...
  {
    name: "yuque_get_doc_metadata",
    description:
      "轻量只读获取已定位文档的完整路径、类型、creator(id/login/name)、created_at、last_editor及updated_at，适用Doc/Sheet/Table。只调用文档详情及定位接口，不额外请求正文或表格行，不返回原始响应。creator取自语雀user_id/user，特殊转移或导入场景不保证代表最初作者；缺失值为null。统计某人创建的文档应按稳定账号ID和created_at筛选，不能扫描正文人名或使用更新时间代替。",
  ...
  },
  {
    ...
    description:
      "分页读取组织空间Table/laketable数据表的字段与真实行记录，区别于Sheet/lakesheet。包含选项标签、人员名称和原始值；返回完整路径与URL。默认100条，最多5000条；按next_offset继续。可用filter筛选、columns投影、raw=false精简；筛选先完整扫描最多5000行，分页基于匹配结果，优化输出最多40KiB。统计优先yuque_get_table_stats。返回底层记录，不应用网页视图筛选、排序或分组，不读取行详情正文，此读取工具不写入；单条归档使用yuque_preview_archive_table_record，Excel导出使用yuque_create_export_link。",
    inputSchema: {
      type: "object",
      properties: {
        ...
        filter: {
          type: "object",
          description:
            "AND equality filters keyed by exact column ID or unique name. Scalar matches display value; arrays use membership. {id: string} matches a person/option ID. Progress is numeric; null matches empty. Full scan capped at 5000 rows.",
        },
        columns: {
          type: "array",
          items: { type: "string" },
          minItems: 1,
          maxItems: 100,
          description: "Column IDs or unique names to return.",
        },
        raw: booleanProperty(
          "Include raw cell values; false returns display values only. Opt-in compact output capped at 40 KiB; follow next_offset.",
          true,
        ),
      },
      required: ["doc_url"],
  ...
  },
  {
    ...
    description:
      "只读统计组织Table：完整分页扫描后按filter筛选，再分别统计group_by字段分布，不返回行数据。上限5000行/20MiB解析行数据/30秒扫描，每字段最多返回100个分组并报告截断。多选和多人员按成员计数，人员按账号ID区分。不是并发一致的原子快照，超限失败而非输出部分统计。",
    inputSchema: objectSchema(
      {
        doc_url: stringProperty("Full Table URL."),
        sheet_id: stringProperty("Required when multiple sheets exist."),
        group_by: {
          type: "array",
          items: { type: "string" },
          minItems: 1,
          maxItems: 10,
          description: "Column IDs or unique names; independent distributions.",
        },
        filter: {
          type: "object",
          description: "Same AND filter semantics as yuque_get_table.",
        },
      },
      ["doc_url", "group_by"],
    ),
  ...
  },
  {
    ...
    description:
      "预览将组织空间 Table 的单条记录移动归档到同一知识库的另一张 Table。读取源记录，按字段 ID 或唯一同名同类型映射，校验单选/多选标签、日期、进度和人员，返回完整 Diff 与 change_token。之后调用 yuque_confirm_change，自动完成创建、写入、回读核对、删除源记录。确认必须传 diff_digest 和 confirm_deletions=true；strict 仅预览，写入要求 best_effort、WRITE_KILL_SWITCH=false 与 YUQUE_WRITE_ORGANIZATION_OPEN=true。拒绝非空行详情正文和不支持字段；每表最多5000行。部分成功/结果未知时调用状态工具只读对账，禁止盲目重试。",
    inputSchema: {
      type: "object",
      properties: {
        source_doc_url: stringProperty("Full source Table URL."),
  ...
  },
  {
    ...
    description:
      "预览修改个人知识库名称或描述；不修改公开性、所有者或组织归属。需要部署者启用YUQUE_WRITE_PERSONAL_OPEN=true。",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "预览将整篇普通Doc移出活动目录（trashed，不宣称硬删除）。默认关闭；必须启用ALLOW_OBJECT_DELETION=true和YUQUE_WRITE_PERSONAL_OPEN=true。Confirm前须展示完整Diff，提交confirm_deletions=true及原样完整路径；Confirm会先保存7天加密快照。",
    inputSchema: {
      type: "object",
  ...
  },
  {
    ...
    description:
      "预览将整份独立Sheet移出活动目录（trashed，不宣称硬删除）。默认关闭；严格校验Sheet类型与个人Host，必须启用ALLOW_OBJECT_DELETION=true和YUQUE_WRITE_PERSONAL_OPEN=true。Confirm前须展示Diff，提交confirm_deletions=true及原样完整路径；Confirm会先保存7天加密快照。",
    inputSchema: {
      type: "object",
  ...
  },
  {
    ...
    description:
      "生成Doc安全Diff，支持append、replace_section、delete_section和rename。delete_section删除完整命名章节并强制二次确认；含专有块时拒绝删除。首版不开放整篇替换。该工具只生成Preview；strict阻止Confirm，best_effort还需WRITE_KILL_SWITCH=false，并按目标Host启用YUQUE_WRITE_PERSONAL_OPEN=true或YUQUE_WRITE_ORGANIZATION_OPEN=true。",
    inputSchema: {
      type: "object",
      properties: {
        doc_url: stringProperty("Full Yuque document URL."),
...
export async function callTool(
  ...
  switch (name) {
    ...
    case "yuque_get_doc_metadata":
      return deps.client.getDocMetadata(
        employeeId,
    ...
    case "yuque_get_table_stats":
      return deps.client.getTableStats(
        employeeId,
        requireString(args, "doc_url"),
        {
          sheetId: optionalString(args, "sheet_id"),
